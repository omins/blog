...
Suspense를 적용하는 방법을 개략적으로 살펴보자.

### Code splitting

Suspense를 활용한 Code splitting은 조건부로 컴포넌트를 렌더링하는 경우 사용하기 적합하다. 특정 이벤트에 따라 렌더링되는 모달 등의 컴포넌트가 이에 해당한다. React 공식 문서에 있는 [예제](https://codesandbox.io/s/d4fkng?file=/App.js&utm_medium=sandpack)도 조건부로 렌더링하는 경우를 다루고 있다. Next로 개발하는 경우 [`next/dynamic`](https://nextjs.org/docs/app/building-your-application/optimizing/lazy-loading)을 사용할 수 있다.
...
Relay, react-query, SWR 등의 데이터 라이브러리는 Suspense를 지원한다. 비동기 데이터를 불러올 때 `suspense: true`를 설정해주고, Suspense로 감싸면 된다. Suspense라는 메커니즘에 맞게 데이터 라이브러리 author들이 개발하기 때문이다. [react-query](https://tanstack.com/query/v4/docs/react/guides/suspense)의 경우를 살펴보자.
...
`useSuspenseQuery()`를 사용할 수도 있는데, react-query 공식 문서의 [예제](https://codesandbox.io/p/sandbox/github/tannerlinsley/react-query/tree/main/examples/react/suspense)에서는 컴포넌트에서 `useSuspenseQuery()`로 데이터를 불러온다.
...
Suspense는 streaming이 enabled 되지 않은 SSR 환경에서 다루기 까다로웠다. code splitting한 컴포넌트를 조건부로 Client에서 렌더링하는 것은 잘 작동하지만, 데이터 fetching을 Suspense로 감싸도 서버에서 HTML을 렌더링 할 때 fetch가 이루어지기 때문에 Suspense의 역할을 하지 못했다.

아래 예제를 실행해보면 서버에서 data를 모두 fetching한 뒤, 렌더링한다는 것을 알 수 있다.
...
개인적으로 Suspense의 활용도와 이점을 끌어올려준 것이 Streaming이라고 생각한다. [New Suspense SSR Architecture in React 18](https://github.com/reactwg/react-18/discussions/37)에서 Suspense와 [`renderToPipeableStream()`](https://react.dev/reference/react-dom/server/renderToPipeableStream)을 활용한 진일보한 설계 패턴을 설명한다. React conf 2021에서도 이 주제에 대한 좋은 [keynote](https://www.youtube.com/watch?app=desktop&v=pj5N-Khihgc&list=PLNG_1j3cPCaZZ7etkzWA7JfdmKWT0pMsa&index=3)도 있다.
