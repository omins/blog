...
CSS와 Javascript는 렌더링을 blocking하는 리소스다.
...
응답 받은 문서를 파싱까지 완료했다면 실제 화면에 콘텐츠를 렌더해야 한다. 렌더링은 렌더 트리 생성, Layout, Paint, Composite로 나눌 수 있다.

### 렌더 트리 생성 (Style)

브라우저의 렌더러는 DOM과 CSSOM을 조합하여 렌더 트리, 다른 말로 계산된 스타일 트리를 구성한다. DOM 루트 노드부터 '보이는' 모든 노드의 계산된 스타일을 포함한 렌더 트리를 구성한다. 이때 `<script>`, `<head>`와 그 자식 요소처럼 `display: none`이 설정된 보이지 않는 요소는 렌더 트리에서 제외된다. Chromium 소스 코드에서도 기본적으로 아래 요소는 렌더 트리에 포함하지 않음을 확인할 수 있다.
