...
We now only need to type 2 characters instead of 10 (excluding 'git' and the commit message). That's an 80% reduction in characters.
...
Small tips: You can also use shell aliases for Git commands. And also check if your cli tool has built in alias features. For example I'm using GitLab CLI and it has alias feature too.
